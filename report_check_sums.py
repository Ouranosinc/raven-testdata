...
def readme_updater(file_checksums: dict, readme: Union[str, Path], data_folder: Union[str, Path]) -> None:
    """Write the checksum dictionary to the bottom of the README.md file, replacing the existing table."""
    readme = Path(readme)
    ...
    i = None
    for i, line in enumerate(lines):
        if line.startswith("[//]: # (Code below this line is autogenerated by `report_check_sums.py`)"):
            break
    if not i:
        raise ValueError("Could not find the marker for the autogenerated section in README.md.")

    # Insert new checksum table
    lines.insert(i + 1, "### Files\n\n|File | Size | Checksum |\n| ---- | ---- | -------- |\n")
...
def main(update_readme: bool = False, readme: Union[str, Path] = "README.md"):
    ...
    if update_readme:
        readme_updater(file_checksums, readme, data_folder)
